...
- 用户想查看 RobotX 项目状态、历史版本，把某个版本发布到正式链接，或删除远端 RobotX 项目。
...
- 用户要求删除远端 RobotX 项目时，必须先确认要删除的项目 ID/网站名称；没有明确确认前不要执行删除。
...
- 如果用户明确要“删除网站/删除项目/从 RobotX 删除”，这是远端项目删除，不是本地记录删除。必须先确认要删除的项目，再执行 `robotx projects delete --project-id <project_id> --yes --output json`。
- 删除远端项目后，如果当前工作区的 `.robotx/targets.json` 仍有对应记录，再询问或根据用户原意用 `robotx targets remove <记录名> --output json` 清理本地记录。
...
远端项目删除是破坏性操作。只有用户明确要求“删除 RobotX 项目/删除这个网站/不要这个线上项目了”，并且已经确认要删除的项目后，才执行。

先列出项目或本地网站记录，确认项目 ID：
...
删除远端项目不会自动删除本地网站记录。如果用户的意图是“这个文件夹以后也不要再更新这个网站”，再删除本地记录：
...
```text
我会删除 RobotX 上的这个项目。这个操作会让对应线上应用不可用。确认删除后，我再继续。
```
...
执行前必须确认用户真的要删除远端项目；不要把它和 `targets remove` 混用。
...
| `confirmation_required` | “删除项目需要你明确确认。” | 只有用户确认删除远端项目后，才加 `--yes` 重试 |
